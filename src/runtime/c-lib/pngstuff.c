...
static void read_png_file(const char *file_name, uint64_t *_width,
                          uint64_t *_height, png_bytep **_row_pointers,
                          int *alpha) {
  ...
  if (bit_depth != 8)
    abort_(
        "[read_png_file] Unsupported png type, color channels must be 8 bits");
