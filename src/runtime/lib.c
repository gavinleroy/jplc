...
uint8_t parse_bool_type(char *type_str, char **new_type_str) {
  ...
  *new_type_str = type_str;
  return t;
fail:
  fprintf(stderr, "[builtin show] Could not parse boolean type in %s",
          orig_type_str);
  exit(127);
}
...
uint8_t parse_int_type(char *type_str, char **new_type_str) {
  ...
  *new_type_str = type_str;
  return t;
fail:
  fprintf(stderr, "[builtin show] Could not parse integer type in %s",
          orig_type_str);
  exit(127);
}
...
uint8_t parse_float_type(char *type_str, char **new_type_str) {
  ...
  *new_type_str = type_str;
  return t;
fail:
  fprintf(stderr, "[builtin show] Could not parse floating type in %s",
          orig_type_str);
  exit(127);
}
...
uint8_t parse_tuple_type(char *type_str, char **new_type_str) {
  ...
  if (*type_str != '}') {
    ...
    while (*type_str != '}') {
      ...
      if (i >= BOOL) {
        fprintf(stderr,
                "[builtin show] Tuples with %zu or more fields not supported",
                i);
        exit(127);
      }
    }
  ...
  *new_type_str = type_str;
  return p;
fail:
  fprintf(stderr, "[builtin show] Could not parse tuple type in %s",
          orig_type_str);
  exit(127);
}
  ...
  while (1) {
    ...
    case '[':
      ...
      while (*type_str == ' ')
        type_str++;
      if (*type_str != ']') {
        fprintf(stderr, "[builtin show] Could not parse type in %s",
                orig_type_str);
        exit(127);
      }
      type_str++;
...
void show_array(uint8_t subtype, int rank, uint64_t *data2) {
  ...
  for (int i = 0; i < rank; i++) {
    ...
    if (__builtin_mul_overflow(size, dim, &size)) {
      fprintf(stderr,
              "[builtin show] Overflow when computing total size of array");
      exit(127);
    }
  }
...
int32_t show(char *type_str, void *data) {
  ...
  while (*type_str == ' ')
    type_str++;
  if (*type_str != '\0') {
    fprintf(stderr, "[builtin show] Cannnot parse type string in %s",
            orig_type_str);
    exit(127);
  }
  show_type(t, data);
...
int32_t _show(char *type_str, void *data) { return show(type_str, data); }
...
void _fail_assertion(char *s) { return fail_assertion(s); }

void print(char *s) { printf("%s", s); }

void _print(char *s) { return print(s); }
...
double _get_time(void) { return get_time(); }

int64_t sub_ints(int64_t a, int64_t b) { return a - b; }

int64_t _sub_ints(int64_t a, int64_t b) { return sub_ints(a, b); }

double sub_floats(double a, double b) { return a - b; }

double _sub_floats(double a, double b) { return sub_floats(a, b); }
  ...
  for (long i = 0; i < x; ++i) {
    for (long j = 0; j < y; ++j) {
      ...
      r = (1.0 - oif) * (1.0 - ojf) * INDX(p, oii, oji, 0) +
          (oif > 0.0 ? (0.0 + oif) * (1.0 - ojf) * INDX(p, oii + 1, oji, 0)
                     : 0.0) +
          (ojf > 0.0 ? (1.0 - oif) * (0.0 + ojf) * INDX(p, oii, oji + 1, 0)
                     : 0.0) +
          (oif * ojf > 0.0
               ? (0.0 + oif) * (0.0 + ojf) * INDX(p, oii + 1, oji + 1, 0)
               : 0.0);
      g = (1.0 - oif) * (1.0 - ojf) * INDX(p, oii, oji, 1) +
          (oif > 0.0 ? (0.0 + oif) * (1.0 - ojf) * INDX(p, oii + 1, oji, 1)
                     : 0.0) +
          (ojf > 0.0 ? (1.0 - oif) * (0.0 + ojf) * INDX(p, oii, oji + 1, 1)
                     : 0.0) +
          (oif * ojf > 0.0
               ? (0.0 + oif) * (0.0 + ojf) * INDX(p, oii + 1, oji + 1, 1)
               : 0.0);
      b = (1.0 - oif) * (1.0 - ojf) * INDX(p, oii, oji, 2) +
          (oif > 0.0 ? (0.0 + oif) * (1.0 - ojf) * INDX(p, oii + 1, oji, 2)
                     : 0.0) +
          (ojf > 0.0 ? (1.0 - oif) * (0.0 + ojf) * INDX(p, oii, oji + 1, 2)
                     : 0.0) +
          (oif * ojf > 0.0
               ? (0.0 + oif) * (0.0 + ojf) * INDX(p, oii + 1, oji + 1, 2)
               : 0.0);
      a = 1.0;
...
#define TEST_PARSE(S)                                                          \
  mem.i = 0;                                                                   \
  printf("[%lu] ", strlen(S));                                                 \
  print_uint8_ts(S, parse_type(S, &junk));
